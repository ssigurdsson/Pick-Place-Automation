...
offsetx, offsety, offsetz = -1650 + 15, -1550 + 45, 8160 #Set position of the nozzle (corrected for nozzle/purple-cross discrepancy (15,45) - CHECK THIS EVERY TIME)
poffsetx, poffsety, poffsetz, ppitch = -950, 1150, 8160, 50 #Set position & pitch of placement locations
...
filenames = ['botfile', 'bpartsfile', 'topfile', 'tpartsfile'] #topfile.fdr, botfile.fdr, tpartsfile.pts, bpartsfile.pts
...
time.sleep(3) #initializing sequence
...
#find purple cross center on the screenshot
for yp in range(300,1000):
    for xp in range(300,1000):
        if np.all(imginit[yp][xp] == [250, 0, 250]):
                if np.all(imginit[yp][xp+1] == [250, 0, 250]) and np.all(imginit[yp+1][xp] == [250, 0, 250]):
                    initcenterx, initcentery = xp, yp
                    break
...
#Updates the device coordinates in the field of view 
def update_device_cords():
    img = pya.screenshot()
    img = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
    nnn = (gpx + d_plane_y*gpy//100)//100
    file_name = "result%d.png" % nnn
    ...
    #Deletes the 1-pixel wide purple crosshair (fade to chip or background color)
    def purple_line_towhite(imagein):
    ...
    def find_device_contours(imagein):
        ...
        for ctr in contour_sizes:
            if minarea < ctr[0] < maxarea: filtered_contours.append(ctr[1])
        return filtered_contours



    def find_devices(imagergb):
        imagergb = purple_line_towhite(imagergb)
        ...
        for ctr in device_contours:
            ...
            if max(rect[1])/min(rect[1]) < 1.13:
                ...
                stdev = np.std(device_surface)
                if stdev > surface_variance: 
                    cv2.putText(overlayed, ".", center, cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.1, (0, 0, 255), 1)
                else: cv2.putText(overlayed, ".", center, cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.1, (255, 0, 0), 1)
                #Dont include chips in overexposed area
                if cy > 60: centers.append([cx, cy, int(stdev > surface_variance)])
...
while 1:
    device_cords = update_device_cords()
    ...
    if gpx < d_plane_x:
        ...
        for nn in range(d_plane_x//unit_move):
            gpx -= unit_move
